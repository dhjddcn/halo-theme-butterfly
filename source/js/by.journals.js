...
const journalsContext = {
    initTagBg() {
        ...
        const colors = [
            ...
            "linear-gradient(to left, rgb(106, 17, 203) 0%, rgb(37, 117, 252) 100%)",
            ...
            "linear-gradient(to right, rgb(233, 222, 250) 0%, rgb(251, 252, 219) 100%)",
    ...
    initLike() {
        ...
        // 获取已经点赞的ids
       var likedIds = JSON.parse(localStorage.getItem("butterfly.likes.journal.ids") || "[]");
        ...
        if ($allItems.length) {
            $allItems.each(function (_, item) {
                ...
                // 判断是否被点赞
                let flag = likedIds.includes(cid);
                const $like = $this.find('.by_journal_likes')
                const $iconlike = $this.find(".by_icon_xihuan-fill");
				const $iconUnlike = $this.find(".by_icon_xihuan");
                if (flag) {
                    $like.addClass('by_journal_liked');
                    $iconlike.show();
                    $iconUnlike.hide();
				} else {
                    $like.removeClass('by_journal_liked');
                    $iconlike.hide();
                    $iconUnlike.show();
				}
                ...
                $like.on('click',function(e){
                    if (likedIds.includes(cid)) return;
                    let loading = false;
					if (loading) return;
                    const xhr = new XMLHttpRequest();
                    xhr.open('POST', "/api/content/journals/" + cid + "/likes");
                    xhr.onload = () => {
                        // 更新点赞列表
                        likedIds = [...likedIds, cid];
                        localStorage.setItem('butterfly.likes.journal.ids', JSON.stringify(likedIds));
                        ...
                        // 更新点赞数
                        const likesNode = document.querySelector("[data-journal-id-likes=\"" + cid + "\"]");
